...
<table cellpadding="0" cellspacing="0" width="100%" height="100%">
    ...
    <tr>
        <td height="100%" class="stretch" valign="top">
			...
			<form name="listings" method="POST" action="test_1.php">
...
// region check for query choice - if ALL then show everything
	if ($region=='all'){
		$query2="select providerid, id, providername, sitekey, providercity, providerstatecode from provider where id in (".$string2.") and 
			...
			$results = $mysqli->query("select providerid, id, providername, sitekey, providercity, providerstatecode from provider where id in (".$string2.") and 
			providerstatus = 'Free' and 
			providername <> '' and 
			sitekey like '".strtoupper($site)."' and 
			providercountrycode = '".$country_code."'");
...
// increment the row counter
	...
	echo("</ol>");
echo("<div class=\"animation_image\" style=\"display:none\">");
echo("<image src=\"ajax-loader.gif\">");
echo("</div>");
