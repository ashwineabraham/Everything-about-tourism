...
// otherwise the www is issing and the first array value holds the site name
    ...
    if ($site_image='127.0.0.1'){
        $site_image="worlddisabilitysearchengine";
    }
...
<?php
	...
	if ($region=='all'){
		$query2="select providerid, id, providername, sitekey, providercity, providerstatecode from provider where providerid in (".$string2.") and 
	...
	while ($row2 = mysql_fetch_array($result2)) {
		...
		$query3=mysql_query("select website from website where providerid='".$row2["id"]."';");
