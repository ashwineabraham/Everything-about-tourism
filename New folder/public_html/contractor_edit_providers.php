...
// get the user ID associated with this hash - dont want to pass as a cookie value in case it might be spoofed
	$query="select distinct(user_id) as user_id from user_log where md5='".$_COOKIE["admin_md5"]."';";
    echo $query."<p>";
...
// if the user has chosen to alter the provider visibility
	if (isset($_GET["shown"])){
		mysql_query("update new_entries set provider_visible='".$_GET["shown"]."' where id = '".$_GET["provider_id"]."';");
	    mysql_query("insert into user_log set user_id='".$user_id."', action='PROVIDER ".$_GET["provider_id"]." VISIBILITY CHANGED', timestamp='".$time_now."', md5='".$_COOKIE["admin_md5"]."';");
