...
// get the user ID associated with this hash - dont want to pass as a cookie value in case it might be spoofed
	$query="select sitekey_represented from authorised_users where id='".$user_id."';";
    $result=mysql_query($query);
    while ($row = mysql_fetch_array($result)) {

// store the user ID for later
		$sitekey_represented=$row["sitekey_represented"];
	}
	...
	while ($row = mysql_fetch_array($result)) {
		...
		if ($row['unapproved_count']>0){
			...
			$query2="select * from new_entries where approved='no' and business_name > '' and address > '' and city > '' and site_deleted != 'yes' and site_category like '".$sitekey_represented."' order by business_name asc;";
